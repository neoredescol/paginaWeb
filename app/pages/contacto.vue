...
const handlePointerMove = (event: PointerEvent) => {
  if (!backgroundWord.value || window.innerWidth <= 800) return
...
@media (max-width: 800px) { .contact-page { min-height: auto; overflow: hidden; } .contact-video__media { opacity: .9; object-position: 58% center; } .contact-video__overlay { background: linear-gradient(180deg,rgba(13,17,23,.55),rgba(13,17,23,.7)); } .contact-shell { display: block; width: calc(100% - (2 * var(--page-padding))); padding: 2.5rem 0 4.5rem; } .contact-title { font-size: clamp(3rem,13.3vw,4.35rem); line-height: .93; } .contact-intro { width: 100%; margin-top: 1.5rem; font-size: .82rem; } .contact-channels { margin-top: 4rem; } .contact-method + .contact-method { margin-top: 2.5rem; } .contact-method a { font-size: clamp(1.05rem,5vw,1.35rem); } .contact-method__icon { width: 2.85rem; height: 2.85rem; } .contact-method__identity { gap: .8rem; } .contact-socials { margin-top: 3.5rem; } .contact-ambient__word { top: 22%; font-size: 30vw; } .contact-ambient__line--one { top: 17%; left: 20%; width: 90%; } .contact-ambient__line--two { right: 25%; bottom: 12%; width: 90%; } }
@media (max-width: 370px) { .contact-title { font-size: 2.8rem; } .contact-method a { gap: .75rem; font-size: .98rem; } }
